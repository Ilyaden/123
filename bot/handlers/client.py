...
async def cm_buy(message : types.Message):
	await makeorder_db.sql_delete(message.from_user.username)
...
async def add_name(callback_query: types.CallbackQuery, state:FSMContext):
	...
	name_list = await makeorder_db.sql_read_name(callback_query.from_user.username)
	try:
		lenght = 0
...
async def give_answer2(message : types.Message, state:FSMContext):
	...
	if (data['answer2'] == 'да'):
		...
		a =[username, ','.join(name)+":", ','.join(number)]
